...
// [длительность мс, задержка мс, пиковая высота 0–1] на бар.
// Отрицательная задержка стартует анимацию уже в середине цикла —
// без этого все бары синхронно взлетали бы с нуля при первом плей.
// Значения подобраны руками, не по формуле: живой рисунок важнее точности.
const EQ_BARS: [number, number, number][] = [
  [780, -120, 0.9], [960, -300, 0.65], [700, -50, 1], [1120, -600, 0.7],
  [850, -220, 0.85], [980, -400, 0.6], [730, 0, 0.95], [1050, -180, 0.75],
  [890, -340, 1], [1200, -500, 0.68], [760, -90, 0.88], [1000, -260, 0.72],
  [820, -410, 0.93], [1150, -60, 0.6], [940, -330, 0.8], [700, -150, 1],
  [1080, -470, 0.7], [870, -20, 0.9], [990, -380, 0.65], [810, -140, 0.97],
  [1130, -290, 0.72], [750, -430, 0.85],
]
...
export function Player() {
  ...
  return (
    <Shell>
      ...
      {/* Второй таймлайн — эквалайзер. Чисто декоративный: не скрабится
          и не отражает реальный спектр, только состояние play/pause.
          50px сверху (от основного таймлайна) и снизу (до кнопки плей) —
          по запросу. */}
      <div className="eq mt-[50px] h-11" data-playing={playing}>
        {EQ_BARS.map(([duration, delay, peak], i) => (
          <div
            key={i}
            className="eq-bar"
            style={
              {
                '--eq-duration': `${duration}ms`,
                '--eq-delay': `${delay}ms`,
                '--eq-peak': peak,
